...
interface Props {
  ...
  onTestRowNumberChange: (value: number | null) => void;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <select
            className={inputCls}
            value={testMode}
            onChange={({ target }) => onTestModeChange(target.value as 'row' | 'manual')}
          >
      ...
      {testMode === 'row' ? (
        <div>
          ...
          <select
            ...
            onChange={({ target }) =>
              onTestRowNumberChange(target.value ? parseInt(target.value, 10) : null)
            }
          ...
          {template.variables.map((variable) => (
            <div key={variable.id}>
              ...
              <input
                ...
                onChange={({ target }) => updateManualVariable(String(variable.id), target.value)}
