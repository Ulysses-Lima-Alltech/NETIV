...
export function TemplateSelector({ templates, selectedTemplateKey, onChange }: Props) {
  const selected = templates.find((item) => item.key === selectedTemplateKey) ?? null;
  return (
    <section className="bg-white border border-[#E5E7EB] rounded-[12px] p-4 space-y-3">
      ...
      <select className={inputCls} value={selectedTemplateKey} onChange={(e) => onChange(e.target.value)}>
