...
import { corretoresApi, projectsApi, type Corretor, type BrokerAvailability } from '../api/client';
...
export function CorretoresPage() {
  ...
  useEffect(() => {
    if (editingId != null && modalOpen) {
      corretoresApi.getAvailability(editingId).then((d) => setAvailability(d.availability)).catch(() => setAvailability([]));
  ...
  const addSlot = () => {
    ...
    const st = newSlotStart.length === 5 ? newSlotStart : newSlotStart.slice(0, 5);
    const et = newSlotEnd.length === 5 ? newSlotEnd : newSlotEnd.slice(0, 5);
    corretoresApi.createAvailability(editingId, { weekday: newSlotWeekday, startTime: st, endTime: et, active: true })
      .then(() => corretoresApi.getAvailability(editingId).then((d) => setAvailability(d.availability)))
      .catch(() => {});
  };
  ...
  const saveEditSlot = () => {
    ...
    corretoresApi.updateAvailability(editingId, editingSlotId, {
      ...
      .then(() => {
        corretoresApi.getAvailability(editingId!).then((d) => setAvailability(d.availability));
        setEditingSlotId(null);
      })
      .catch(() => {});
  };
  ...
  const removeSlot = (availabilityId: number) => {
    ...
    corretoresApi.deleteAvailability(editingId, availabilityId)
      .then(() => corretoresApi.getAvailability(editingId).then((d) => setAvailability(d.availability)))
      .catch(() => {});
  };
  ...
  const save = () => {
    ...
    if (!n) { setErr('Nome completo é obrigatório.'); return; }
    setSaving(true);
    setErr(null);
    if (editingId != null) {
      corretoresApi
        .update(editingId, { fullName: n, city: city.trim(), phone: phone.trim(), realEstateAgency: realEstateAgency.trim(), active, enterpriseIds: selectedEnterpriseIds })
  ...
  const handleInactivate = (id: number) => {
    ...
    corretoresApi.inactivate(id).then(() => loadList()).catch(() => {});
  };
        ...
        <div className="max-w-[1200px] mx-auto flex items-center gap-4 px-6 h-14">
          <div className="flex items-center gap-2 p-1.5 rounded-[12px] bg-[#F3F4F6]/60 border border-[#E5E7EB]/80">
            <Link to="/inbox" className={navBtn('/inbox')}>
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="mr-1.5 shrink-0"><path d="m15 18-6-6 6-6"/></svg>
              Inbox
            </Link>
          ...
          <h1 className="text-[15px] font-semibold text-[#111827]">Corretores</h1>
        </div>
      </header>

      <div className="max-w-[1200px] mx-auto px-6 py-8">
        <div className="flex justify-between items-center mb-6">
          <p className="text-[13px] text-[#6B7280]">Cadastro de corretores para uso na fila de atendimentos.</p>
        ...
        <div className={card}>
          {loading ? (
            ...
            <div className="py-12 text-center">
              <p className="text-[15px] font-medium text-[#111827] mb-1">Nenhum corretor cadastrado</p>
              <p className="text-[13px] text-[#6B7280] mb-4">Clique em &quot;Novo corretor&quot; para começar.</p>
              <button type="button" onClick={openNew} className={btnPrimary}>Novo corretor</button>
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-[#E5E7EB]">
                    <th className="py-3 px-4 text-[11px] font-bold text-[#9CA3AF] uppercase tracking-wider">Nome</th>
                    ...
                    <th className="py-3 px-4 text-[11px] font-bold text-[#9CA3AF] uppercase tracking-wider">Imobiliária</th>
                ...
                <tbody>
                  {list.map((c) => (
                    <tr key={c.id} className="border-b border-[#F3F4F6] hover:bg-[#F9FAFB]">
                      <td className="py-3.5 px-4 text-[14px] font-medium text-[#111827]">{c.fullName}</td>
                      <td className="py-3.5 px-4 text-[13px] text-[#6B7280]">{c.city || '—'}</td>
                      <td className="py-3.5 px-4 text-[13px] text-[#6B7280]">{c.phone || '—'}</td>
                      <td className="py-3.5 px-4 text-[13px] text-[#6B7280]">{c.realEstateAgency || '—'}</td>
                      <td className="py-3.5 px-4">
                        <span className={`inline-flex text-[11px] font-semibold px-2.5 py-1 rounded-md ${c.active ? 'bg-[#D1FAE5] text-[#059669]' : 'bg-[#F3F4F6] text-[#6B7280]'}`}>
                          {c.active ? 'Ativo' : 'Inativo'}
                        </span>
                      </td>
                      <td className="py-3.5 px-4">
                        <button type="button" onClick={() => openEdit(c)} className={btnGhost + ' text-[12px] mr-2'}>
                          Editar
                        </button>
                        {c.active && (
                          <button type="button" onClick={() => handleInactivate(c.id)} className="text-[12px] font-medium text-[#9CA3AF] hover:text-red-600 transition-colors">
                            Inativar
                          </button>
                        )}
                      </td>
          ...
          <div className={`${card} w-full max-w-md shadow-xl max-h-[90vh] flex flex-col overflow-hidden`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <label>
                ...
                <input className={field} value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Ex.: João Silva" />
              </label>
              <label>
              ...
              <label>
                ...
                <input className={field} value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Ex.: (11) 99999-9999" />
              </label>
              <label>
                <span className={label}>Imobiliária</span>
                <input className={field} value={realEstateAgency} onChange={(e) => setRealEstateAgency(e.target.value)} placeholder="Ex.: Imobiliária XYZ" />
              </label>
              <div>
                <span className={label}>Empreendimentos atendidos</span>
                <p className="text-[12px] text-[#9CA3AF] mb-2">Selecione os empreendimentos que este corretor pode atender.</p>
                <div className="max-h-32 overflow-y-auto rounded-[8px] border border-[#E5E7EB] p-2 space-y-1.5">
                  {projects.length === 0 ? (
                    <p className="text-[12px] text-[#9CA3AF] py-2">Nenhum empreendimento cadastrado.</p>
                  ) : (
                    projects.map((p) => (
                      <label key={p.id} className="flex items-center gap-2 cursor-pointer hover:bg-[#F9FAFB] rounded px-2 py-1">
                        <input
                          type="checkbox"
                  ...
                  <div className="space-y-2">
                    {availability.map((s) => (
                        <div key={s.id} className="flex items-center gap-2 flex-wrap">
                          {editingSlotId === s.id ? (
                            <>
                              <select className={field + ' flex-1 min-w-0'} value={editSlotWeekday} onChange={(e) => setEditSlotWeekday(Number(e.target.value))}>
                                {WEEKDAYS.map((d, i) => (
                                  <option key={i} value={i}>{d}</option>
                                ))}
                              </select>
                              <input type="time" className={field + ' w-24'} value={editSlotStart} onChange={(e) => setEditSlotStart(e.target.value)} />
                              <span>-</span>
                              <input type="time" className={field + ' w-24'} value={editSlotEnd} onChange={(e) => setEditSlotEnd(e.target.value)} />
                              <label className="flex items-center gap-1">
                                <input type="checkbox" checked={editSlotActive} onChange={(e) => setEditSlotActive(e.target.checked)} />
                                <span className="text-[12px]">Ativo</span>
                              </label>
                              <button type="button" onClick={saveEditSlot} className={btnGhost + ' text-[12px]'}>Salvar</button>
                              <button type="button" onClick={() => setEditingSlotId(null)} className="text-[12px] text-[#9CA3AF]">Cancelar</button>
                            </>
                          ) : (
                            <>
                              <span className="text-[13px] flex-1">{WEEKDAYS[s.weekday]} {s.startTime.slice(0, 5)}–{s.endTime.slice(0, 5)} {!s.active && '(inativo)'}</span>
                              <button type="button" onClick={() => startEditSlot(s)} className={btnGhost + ' text-[12px]'}>Editar</button>
                              <button type="button" onClick={() => removeSlot(s.id)} className="text-[12px] text-[#9CA3AF] hover:text-red-600">Remover</button>
                            </>
                          )}
                        </div>
                      ))}
                      <div className="flex flex-wrap gap-2 pt-2 border-t border-[#E5E7EB]">
                        ...
                        <button type="button" onClick={addSlot} className={btnGhost + ' text-[12px]'}>Adicionar horário</button>
                      </div>
                    </div>
                  </div>
                  <label className="flex items-center gap-2">
                    <input type="checkbox" checked={active} onChange={(e) => setActive(e.target.checked)} className="rounded border-[#E5E7EB]" />
