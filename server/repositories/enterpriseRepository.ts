...
// Knowledge Backfill Types and Functions
export interface KnowledgeFileBackfillTarget {
  enterpriseId: number;
  enterpriseName: string;
  enterpriseCity: string | null;
...
export interface KnowledgeFileBackfillRow {
  enterprise_id: number;
  enterprise_name: string;
  enterprise_city: string | null;
  file_id: number;
...
export async function loadKnowledgeFileBufferForBackfill(row: KnowledgeFileBackfillRow): Promise<{ ok: true; buffer: Buffer } | { ok: false; reason: string }> {
...
export async function reindexKnowledgeFileForBackfill(
  ...
  try {
    const text = loaded.buffer.toString('utf-8');
    ...
    // Simulação de chunking - na implementação real usaria o serviço de chunking
    const chunkSize = 1000;
    const chunksGenerated = Math.ceil(text.length / chunkSize);
    ...
    // Na implementação real, aqui seria feita a reindexação dos chunks no banco
    return {
      enterpriseId: row.enterprise_id,
      enterpriseName: row.enterprise_name,
      fileId: row.file_id,
