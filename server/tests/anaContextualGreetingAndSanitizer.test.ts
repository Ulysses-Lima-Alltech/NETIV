...
test('caso 2: sem empreendimento identificado, resposta deve continuar atendimento e nao cair em saudacao isolada', () => {
  const guard = evaluateAnaEmptyFallbackGuard({
    reply: 'Oi, tudo bem sim! E com voce? Voce prefere me contar a cidade ou o empreendimento de interesse?',
    userMessage: 'oi, tudo bem?',
...
test('caso 3: pergunta explicita sobre Evora nao deve induzir triagem generica', () => {
  const guard = evaluateAnaEmptyFallbackGuard({
    reply: 'O Evora e um loteamento fechado em Atibaia com foco em seguranca e lazer. Quer que eu detalhe valores, localizacao ou visita?',
...
test('caso 5: frase generica proibida nao existe hardcoded no projeto', () => {
  const rootFiles = [
    new URL('../utils/anaReplyFinalize.js', import.meta.url),
    new URL('../services/conversationEngine.js', import.meta.url),
    new URL('../services/anaAgentService.js', import.meta.url),
  ];
