...
test('log canonico inclui contactId e somente metadados permitidos', () => {
  ...
  assert.equal(entries[0]?.[0], '[ANA_AUTOMATION_BLOCKED_HANDOFF]');
...
test('retry e workers encerram HANDOFF sem reagendar e usam o guard final comum', () => {
  ...
  for (const file of [
    ...
    'services/anaOutboundQuotaService.ts',
  ]) {
...
test('batch mantem template inicial e aplica postSendMode HANDOFF depois', () => {
  ...
  assert.ok(existingConversationLookup >= 0 && send > existingConversationLookup && routing > send);
  assert.match(source, /if \(params\.postSendMode !== 'HANDOFF'\)/);
  assert.match(source, /automationType: 'batch_template_non_handoff'/);
