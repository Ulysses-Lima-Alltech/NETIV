...
import {
  buildCanonicalLazerFullReply,
  buildEvoraFirstReplySafeFallback,
  normalizeAnaLocalTextForRules,
  isGratitudeOnlyMessage,
  isGenericFirstGreetingMessage,
  isFirstContactGeneralInterestMessage,
  isFirstContactEnterpriseInterestMessage,
  hasEnterprisePresentationContent,
  isFirstReplyGreetingOnlyMessage,
  buildFirstGreetingSafeFallback,
  isGenericInterestFollowup,
  isConversationalGenericFollowup,
  buildConversationalCanonicalContext,
  hasUnauthorizedPriceClaimInConversationalReply,
  buildConversationalCanonicalFallback,
} from '../utils/anaEvoraGreetingAndFollowup.js';
import {
  normalizeListItemForCompare,
  dedupeListItems,
  extractReplyListItems,
} from '../utils/anaReplyListItems.js';
import {
  countEvoraRegionCoreSignals,
  dedupeMessageParts,
  scrubClientVisibleBrandLeak,
  splitRhetoricalSeparatorsForWhatsApp,
...
import {
  hasUnsupportedLocationPromise,
  isLocationLinkRequest,
  isImageMaterialRequest,
  ANA_IMAGE_NOT_FOUND_REPLY,
  isVideoMaterialRequest,
  isProactiveVideoOfferIntent,
  pickAuthorizedLocationLink,
  pickAuthorizedLocationAddress,
  buildEvoraLocationOverview,
  buildEvoraRegionCanonicalReply,
  buildEvoraAddressCanonicalReply,
  getEvoraCanonicalMapsLink,
  pickLocationLinkFromKnowledge,
  sanitizeEvoraRestrictedKnowledgeForAna,
  hasConversationalUnsupportedPromise,
  isBrokenEnumeratedReply,
} from '../utils/anaEvoraLocationAndMaterial.js';
import {
  formatYmdForAnaVisitAvailability,
  addDaysYmdForAnaVisitAvailability,
} from '../utils/anaVisitAvailabilityDates.js';
import {
  containsForbiddenMissingDetailFallbackText,
  normalizeIntentText,
  toFirstName,
  isWeakEntregaAnswer,
  sanitizeInternalInstructionLeakText,
  hasAnaInternalInstructionLeak,
...
import {
  looksLikeStandaloneNameReply,
  isAppointmentContextualQuestion,
  isShortGenericFollowUpMessage,
  isAffirmativeShortReply,
  isPendingFollowupContinuationRequest,
  extractFollowupTopicsFromAssistantQuestion,
  followupTopicLabel,
} from '../utils/anaFollowupContinuationDetection.js';
import {
...
async function handleIncomingMessageCore(ctx: IncomingMessageContext): Promise<void> {
  ...
  let activeWhatsAppNoEnterpriseForTurn = false;
  let assistantReplyAttemptedOrSent = false;
  let hardBlockReasonForTurn: string | null = null;
  ...
  const getAnaSilentExitContext = () => {
    ...
    return {
      ...
      hardBlockReason,
      activeWhatsAppNoEnterprise: activeWhatsAppNoEnterpriseForTurn || activeWhatsAppNoEnterprise,
      assistantReplyAttemptedOrSent,
    };
  };
  const sendGlobalNoEnterpriseFinalSafeReply = async (reason: string | null): Promise<boolean> => {
    ...
    if (assistantReplyAttemptedOrSent) return false;
    ...
    console.log('[ANA_GLOBAL_NO_ENTERPRISE_FINAL_SAFE_REPLY]', {
      ...
      handoff: silentExitContext.handoff,
      manualClosedAt: silentExitContext.manualClosedAt,
    ...
    console.log('[ANA_GLOBAL_NO_ENTERPRISE_FINAL_SAFE_REPLY_SENT]', {
      conversationId,
      reason: reason ?? 'unknown_silent_exit',
  ...
  const verifyAndRepairHandoffAfterBrokerAssignment = async (
    ...
    if (ANA_HANDOFF_DISABLED && convAfterAssignment.assigned_broker_id != null) {
      console.log('[ANA_HANDOFF_MUTATION_BLOCKED]', {
        ...
        assignedBrokerId: convAfterAssignment.assigned_broker_id,
      });
    }
  ...
  try {
    ...
    if (effectiveConv.classification === 'Carteira' || effectiveConv.manual_closed_at != null) {
      ...
      hardBlockReasonForTurn = blockedReason;
      assistantReplyAttemptedOrSent = true;
    ...
    console.log('[ANA_ACTIVE_WHATSAPP_NO_ENTERPRISE_GUARD]', {
      ...
      handoff: Boolean(effectiveConv.handoff),
      manualClosedAt: effectiveConv.manual_closed_at != null,
    ...
    if (handoffDisabledBrokerIntent) {
      console.log('[ANA_HANDOFF_MUTATION_BLOCKED]', {
        ...
        explicitBrokerRequest: rawExplicitBrokerRequest,
    ...
    if (!aiApiKey) {
      hardBlockReasonForTurn = 'missing_resolved_api_key_after_gate';
      assistantReplyAttemptedOrSent = true;
      anaTurnAuditOutcome = 'blocked';
...
console.log('[ANA_QWEN_GUARDRAIL_DECISION]', {
    ...
    if (anaTurnAuditOutcome === 'silent' || anaTurnAuditOutcome === 'blocked') {
      ...
      console.log('[ANA_SILENT_EXIT_BLOCKED]', {
        ...
        returnReason: anaTurnAuditBlockedReason ?? 'unknown_silent_exit',
        enterpriseId: finalSilentExitContext.enterpriseId,
        channel: finalSilentExitContext.channel,
        ...
        activeWhatsAppNoEnterprise: finalSilentExitContext.activeWhatsAppNoEnterprise,
        assistantReplyAttemptedOrSent: finalSilentExitContext.assistantReplyAttemptedOrSent,
